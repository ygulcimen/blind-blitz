-- Create a logging table for payment debugging
CREATE TABLE IF NOT EXISTS payment_debug_logs (
  id BIGSERIAL PRIMARY KEY,
  room_id UUID,
  log_level TEXT, -- 'INFO', 'ERROR', 'DEBUG'
  message TEXT,
  error_code TEXT,
  error_detail TEXT,
  room_status TEXT,
...
-- Enable RLS but allow all access for debugging
ALTER TABLE payment_debug_logs ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow all access to payment logs" ON payment_debug_logs
FOR ALL TO public
USING (true)
WITH CHECK (true);

-- Update the payment function with detailed logging
...
DO $$
BEGIN
  RAISE NOTICE '✅ Payment logging enabled!';
  RAISE NOTICE '   - Created payment_debug_logs table';
  RAISE NOTICE '   - Updated charge_entry_fees_and_start_game function with logging';
  RAISE NOTICE '   - Query logs with: SELECT * FROM payment_debug_logs ORDER BY created_at DESC;';
END $$;
